...
%token <node> ID WHILE IF ELSE NUMBER TRUE FALSE NU THIS NEW READ WRITE WRITELN VOID CLASS INT BOOLEAN IDD
...
nosignfactor: '(' expression ')' {  $<node>$=$<node>2; }
	...
	| '!' factor		 { int j=0; $<node.place>$=strdup("!"); strcat($<node.place>$,$<node.place>2); 
				   for(j=0;$<node.code[j]>2!=NULL;j++) $<node.code[j]>$=$<node.code[j]>2; }
	| '(' RefType ')' nosignfactor {$<node.place>$=strdup("("); strcat($<node.place>$,$<node.place>2); strcat($<node.place>$,")");
					strcat($<node.place>$,$<node.place>4);/*may write some code*/}
	| IDD ')' nosignfactor {        $<node.place>$=strdup($<node.place>2); strcat($<node.place>$,")");
					strcat($<node.place>$,$<node.place>3);/*may write some code*/}
