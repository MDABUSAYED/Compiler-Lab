...
stat: statment     { $<node>$=$<node>1;  }
	...
	;
stat1: statment1   { $<node>$=$<node>1;  }
	| stat1 ',' statment1    { int i=0,j=0; for(i=0;$<node.code[i]>1!=NULL;i++) $<node.code[i]>$=$<node.code[i]>1;
				 for(j=0;$<node.code[j]>3!=NULL;j++) $<node.code[i+j]>$=$<node.code[j]>3;
				 $<node.code[i+j]>$=NULL;
			  	}
;
statment1: ID '=' expression  	 {  int i=0;for(i=0;$<node.code[i]>3!=NULL;i++)   $<node.code[i]>$=$<node.code[i]>3;
...
;
for_stmt: FOR '(' stat1 ';' expression ';' stat1 ')' '\n' statment_block
						      {  int i=0,j=0; for(i=0;$<node.code[i]>3!=NULL;i++) $<node.code[i]>$=$<node.code[i]>3;
							 $<node.code[i]>$=level(); char *str1,*str2; str2=$<node.code[i]>$;
							 for(j=0,i=i+1;$<node.code[j]>5!=NULL;j++,i++) $<node.code[i]>$=$<node.code[j]>5;
							 $<node.code[i]>$=strdup("if ");strcat($<node.code[i]>$,$<node.place>5);str1=level();
							 strcat($<node.code[i]>$,strdup(" == 0 goto ")); strcat($<node.code[i]>$,str1);
							 for(j=0,i=i+1;$<node.code[j]>10!=NULL;j++,i++) $<node.code[i]>$=$<node.code[j]>10;
							 for(j=0;$<node.code[j]>7!=NULL;j++,i++) $<node.code[i]>$=$<node.code[j]>7;
							 $<node.code[i]>$=strdup("goto "); strcat($<node.code[i]>$,str2);
						   	 $<node.code[i+1]>$=strdup(str1); $<node.code[i+2]>$=NULL;
							}
